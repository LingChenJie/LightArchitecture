...
import com.android.architecture.extension.ContextKt;
import com.android.architecture.extension.ResourcesKt;
import com.android.architecture.ui.page.action.AnimAction;
import com.android.architecture.ui.page.action.HandlerAction;
...
public class BaseDialog extends AppCompatDialog implements LifecycleOwner,
    ...
    public void dismiss() {
        ...
        if (focusView != null) {
            InputMethodManager service = ContextKt.getSystemService(InputMethodManager.class);
            if (service != null) {
                service.hideSoftInputFromWindow(focusView.getWindowToken(), 0);
            }
        }
        super.dismiss();
    }
    ...
    public static class Builder<B extends Builder<?>> {
        ...
        public B setGravity(int gravity) {
            // 适配布局反方向
            mGravity = Gravity.getAbsoluteGravity(gravity, ResourcesKt.getResources().getConfiguration().getLayoutDirection());
            if (isCreated()) {
        ...
        public B setText(@IdRes int viewId, @StringRes int stringId) {
            return setText(viewId, ResourcesKt.getString(stringId));
        }
        ...
        public B setHint(@IdRes int viewId, @StringRes int stringId) {
            return setHint(viewId, ResourcesKt.getString(stringId));
        }
